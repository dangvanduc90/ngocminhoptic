<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Authentication Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are used during authentication for various
    | messages that we need to display to the user. You are free to modify
    | these language lines according to your application's requirements.
    |
    */

    'trangchu' => 'Trang chủ',
    'giohang' => 'Giỏ hàng',
    'baiviet' => 'Bài viết',
    'tintuc' => 'Tin tức',
    'khuyenmai' => 'Khuyến mãi',
    'vechungtoi' => 'Về chúng tôi',
    'sanpham' => 'Sản phẩm',
    'datlich' => 'Đặt lịch khám',
    'lienhe' => 'Liên hệ',
    'tracuu' => 'Tra cứu bệnh án',
    'thongtinsp' => 'Thông tin sản phẩm',
    'masp' => 'Mã sản phẩm',
    'chatlieu' => 'Chất liệu',
    'soluong' => 'Số lượng',
    'themvaogiohang' => 'Thêm vào giỏ hàng',
    'mau' => 'Màu',
    'baivietkhac' => 'Bài viết khác',
    'xemnhanh' => 'Xem nhanh',
    'nhaphovaten' => 'Nhập họ và tên',
    'nhapdienthoai' => 'Nhập điện thoại',
    'nhapemail' => 'Nhập email',
    'nhapdiachi' => 'Nhập địa chỉ',
    'noidung' => 'Nội dung',
    'stk' => 'Số tài khoản',
    'chonnganhang' => 'Chọn ngân hàng',
    'gui' => 'Gửi',
    'giodatlich' => 'Giờ đặt lịch',
    'ngaydatlich' => 'Ngày đặt lịch',
    'choncoso' => 'Chọn cơ sở',
    'nhapthongtintimkiem' => 'Nhập thông tin tìm kiếm',
    'hoten' => 'Họ tên',
    'mahoadon' => 'Mã hóa đơn',
    'dienthoai' => 'Điện thoại',
    'ngaykham' => 'Ngày khám',
    'ngaykhamlai' => 'Ngày khám lại',
    'diachi' => 'Địa chỉ',
    'search' => 'Tìm kiếm',
    'stt' => 'STT',
    'xemthem' => 'Xem thêm',
    'xoakhoigiohang' => 'Xóa sản phẩm khỏi giỏ hàng',
    'bancomuonxoa' => 'Bạn có muốn xóa ?',
    'xoa' => 'Xóa',
    'thoat' => 'Thoát',
    'thongtinbenhnhan' => 'Thông tin bệnh nhân',
    'kiemtrathiluc' => 'KẾT QUẢ KHÁM & KIỂM TRA THỊ LỰC',
    'thongsokinhcu' => 'Thông số kính cũ',
    'thiluc' => 'Thị cũ',
    'thongsokinhmoi' => 'Thông số kính mới',
    'thiluccokich' => 'Thị lực có kính',
    'nhanap' => 'Nhãn áp',
    'matphai' => 'Mắt phải',
    'mattrai' => 'Mắt trái',
    'hoadonbanhang' => 'HÓA ĐƠN BÁN HÀNG',
    'dongia' => 'Đơn giá',
    'thanhtien' => 'Thành tiền',
    'tongtienhang' => 'Tổng tiền hàng',
    'khuyenmai' => 'Khuyến mãi',
    'in' => 'Xuất hóa đơn',
    'anhsp' => 'Ảnh sản phẩm',
    'thongtin' => 'Thông tin',
    'xoa' => 'Xóa',
    'tamtinh' => 'Tạm tính',
    'thtt' => 'Tiến hành đặt hàng',
    'nhanhangthanhtoan' => 'Nhận hàng thanh toán',
    'thanhtoannhanhang' => 'Thanh toán nhận hàng',
    'camon' => 'Cảm ơn bạn đã lựa chọn sản phẩm của chúng tôi',
    'ttctsp' => 'Thông tin chi tiết sản phẩm',
    'ttkhac' => 'Thông tin khác',
    'splienquan' => 'Sản phẩm liên quan',
    'doitac' => 'Đối tác',
    'textsan' => 'Sản phẩm nổi bật',
    'export_patient_result' => 'Xuất kết quả khám',

];
