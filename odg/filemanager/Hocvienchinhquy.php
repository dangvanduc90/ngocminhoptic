...
class Hocvienchinhquy extends Model
{
    ...
    protected $fillable = [
        ...
        'hocba', 'bangcntt', 'ltdieuduong', 'ghichu', 'nguontt_id','status',
        
    ];
    protected $appends = ['name_noisinh', 'data_chamsocvien', 'sum_ngaygoilai'];
    ...
    public function getSumNgaygoilaiAttribute()
    {
        $_hoc_vien_id = $this->attributes['id'];
        if ($_hoc_vien_id != null) {
            $_tmp = Datachamsocvien::orderby('id', 'DESC')
            ->where('hocvien_id', $_hoc_vien_id)->where('type', 1)->first();        
            if($_tmp != null){
                if($_tmp->ngaygoilai != null && $_tmp->ngaygoilai != "") return 1;
            }
        }
        return 0;
    ...
    {
        return $this->hasMany('App\Admin\Datachamsocvien','hocvien_id', 'id');
    }
}
